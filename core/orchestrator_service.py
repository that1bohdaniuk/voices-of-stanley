...
class OrchestratorService:
    def __init__(self):
        ...
        self._job_queue = asyncio.Queue(maxsize=100)
    ...
    async def publish(self, sig: Signal):
        sig.timestamp = sig.timestamp or time.time()
    ...
    async def _reducer_loop(self):
        while not self._stop.is_set():
            ...
            if sig.type == SignalType.EVENT_INGESTED:
                ...
                await self._enqueue_once(JobType.RUN_DIRECTOR)
    ...
    async def _scheduler_loop(self):
        while not self._stop.is_set():
            ...
            now = time.time()
            if now - self.state.last_prune_ts > 5 * 24 * 60 * 60:
                await self._enqueue_once(JobType.RUN_PRUNER)
                self.state.last_prune_ts = now

    async def _enqueue_once(self, job_type: JobType):
        # minimal coalescing strategy could track pending flags
        await self._job_q.put(job_type)

    async def _worker_loop(self):
        while not self._stop.is_set():
            job = await self._job_q.get()
